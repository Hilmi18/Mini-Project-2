...
const Navbar = ({ background, trigger, triggerDelete }) => {
  ...
  const content = (
    <>
      <div className="md:hidden block absolute top-16 w-full left-0 right-0 bg-black bg-opacity-40 mt-1 pb-5 transition">
        <ul className="text-center flex flex-col items-center justify-center gap-5 text-xl p-20 h-24  ">
          <Link
            to="/"
            className="text-white text-[17px] cursor-pointer hover:text-red-500"
          >
            <li>Home</li>
          </Link>
          <Link
            ...
            spy={true}
            smooth={true}
            className="text-white text-[17px] p-1 rounded-lg relative"
          >
            <li>
              <FiShoppingCart className="text-[20px]" />
              <span className="bg-red-500 text-white text-[10px] rounded-xl px-2 absolute top-0 right-0 -mt-2 -mr-4 h-4 w-6 flex justify-center items-center">
                {cartItemCount}
              </span>
            </li>
          </Link>
  ...
  return (
    <div
      className={`${transparent} ${background} flex justify-between px-20 py-[15px] pt-11 fixed w-full max-w-[1800px]`}
    >
      ...
      <button
        className="block md:hidden transition text-white z-20"
        onClick={handleClick}
      >
